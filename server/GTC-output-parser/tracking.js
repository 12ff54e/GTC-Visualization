...
class Tracking extends PlotType {
    ...
    constructor(data, basicParams) {
        ...
        stepLoop: do {
            ...
            process.stdout.write(
                `\b\b\b\b\b\b\b${((100 * thisStep) / mstep)
                    .toFixed(2)
                    .padStart(6, ' ')}%`
            );
     ...
     * @param {string} dir
     */
    static async readDataFile(dir, basicParams, nodeModules) {
        let tracks = new Array();
        for (let file of await nodeModules.fs.promises.readdir(dir)) {
            if (file.startsWith('TRACKP')) {
                const fileContent = await nodeModules.promises.fs.readFile(
                    nodeModules.path.join(dir, file),
                    'utf-8'
                );
    ...
    plotData(id, eqData) {
        const figureContainer = Array.from(
            { length: 2 },
            _ => new PlotlyData()
        );
        if (id === 'random_pick') {
            const psiMesh = [...Array(eqData.radialGridPtNum2).keys()].map(
                r => (this.psiw * r) / (eqData.radialGridPtNum2 - 1)
            );
            const thetaMesh = [...Array(eqData.poloidalGridPtNum).keys()].map(
                i => (2 * Math.PI * i) / (eqData.poloidalGridPtNum - 1)
            );
            ...
            figureContainer[0].data.push({
                ...
                hovertemplate:
                    'psi: %{a:.4g}<br>theta: %{b:.4g}<extra></extra>',
            });
            figureContainer[0].axisEqual();
            figureContainer[0].axesLabel = {
                x: '$\\text{R}$',
                y: '$\\text{Z}$',
            };
